...
def ctem_gui():
    ...
    line, = plt.plot(k, ctemh(k, [init_energy, init_C3, 50, init_defocus, 100, 0.5],0), lw=2)
    ...
    def update(val):
        line.set_ydata(f(k, energy_slider.val, C3_slider.val, C5_slider.val, Defocus_slider.val, 
...
def perov_gui():
    ...
    # The function to be called anytime a slider's value changes
    def update(val):
        ctf, psf,im = f(energy_slider.val, C3_slider.val, C12_slider.val, Defocus_slider.val, 
                         semiangle_slider.val, ddf_slider.val)
        wav = wavelen(energy_slider.val)
        rmax = 2.0*np.sqrt( np.sqrt( C3_slider.val*wav*wav*wav) )
        #line1.set_ydata(ctf)
...
def cc_gui():
    ...
    # Create the figure and the line that we will manipulate
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=[10,7])
    ctf, psf = f(k, init_energy, init_C3, init_C5, init_defocus, init_semiangle, init_ddf)
