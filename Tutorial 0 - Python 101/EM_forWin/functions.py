...
def scalebar(data,pixelsize=0,unit='',scale=10):
    if pixelsize == 0:
        pixelsize = data.info['pixelSize']
    if unit == '':
        unit = data.info['unit']
    y, x = (np.shape(data.data))
    target = x/scale*pixelsize
    if unit in ['micron', 'um', 'µm', "u'\xb5m'"]:
        unit = 'µm'
    ...
    font = ImageFont.truetype(fontpath, int(y/40))
